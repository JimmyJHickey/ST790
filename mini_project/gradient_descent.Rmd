...
How do we know that when we stop that we have actually achieved **the** minimum point of our objective function? Before even running the algorithm, we need to prove that there is **a** global minimum. We can do this by showing that our objective function is _strongly convex_ and _coercive_.


We will see later in [Issues with Gradient Descent](#Issues-with-Gradient-Descent) examples of functions that are not strongly convex and coercive and the consequences of running gradient descent on them.

### Convergence via Zangwill-Meyer Theorem
...
We saw in the [Smoothing Time Series Data](##-Smoothing-Time-Series-Data) example that calculating the Hessian can be expensive, and inverting it can be more expensive yet. However, if there is special structure in the Hessian, perhaps it can quick to invert and it might make more sense to use Newton's method. It is important to fully assess the problem that you are investigating before committing to an algorithm.
